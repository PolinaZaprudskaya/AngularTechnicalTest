...
import { FailedDialogComponent } from '@shared/dialogs/failed-dialog/failed-dialog.component';
  ...
  public updateTaskInfo(): void {
    ...
    this.tasksApiService.updateTask(newTask).subscribe({
      next: () => {
        ...
        this.dialogService.open(SuccessDialogComponent, { backdropClick: true });
        this.editMode = false;
        this.task = newTask;
      },
      error: () => {
        this.isLoading = false;
        this.dialogService.open(FailedDialogComponent, { backdropClick: true });
      }
    });
  }
